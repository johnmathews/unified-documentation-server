...
- `/health` — Health check returning overall status (`healthy`/`degraded`/`error`), poll interval, total source/chunk counts, and per-source breakdown (source status [`healthy`/`warning`/`error`/`unknown`], file count, chunk count, last indexed time, last checked time, last error, consecutive failure count, and `repo_url`/`branch` — `repo_url` is the normalised `https://github.com/owner/repo` base when the source's configured path is a github URL, else `null`; the webapp uses these to render a "View on GitHub" link). Per-source status is persisted in a `source_status` SQLite table so it survives server restarts. Also returns `last_ingestion` (diagnostic metrics from the most recent ingestion cycle: duration, RSS peak before/after, per-batch flush timing) and `chat_model_valid` / `chat_model_error` fields populated by a startup probe against the Anthropic API — when `chat_model_valid` is `false`, the chat endpoints short-circuit with HTTP 503.
...
- `/api/tree` (GET) — Legacy category-grouped document tree. Each source has `root_docs` (root-level files like README.md), `docs` (files in subdirectories), `journal` (files under journal/), `engineering_team` (files under .engineering-team/), `learning_journal` (files under learning/), `research` (files under research/), `skills` (files under skills/), `runbooks` (files under runbooks/), and `pdf` (any file ending in .pdf). Retained for backwards compatibility; the webapp's tree views consume the `/api/sources/tree` endpoints below.
- `/api/sources/tree` (GET) — Bulk file listing for every configured source. Returns `{"sources": [{"source": name, "files": [{doc_id, file_path, title, modified_at}, ...]}, ...]}` with parent docs only (chunks excluded). Iterates configured sources rather than only KB-populated ones, so newly-added sources appear in the webapp sidebar before their first scan completes. The webapp builds the nested folder tree client-side by splitting `file_path` on `/`.
- `/api/sources/{name}/tree` (GET) — Same shape but for one source: `{"source": name, "files": [...]}`. Returns 404 if `name` is not in `config.sources` (an unknown source name is strictly an error, not an empty list).
